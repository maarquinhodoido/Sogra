...
export function AdminSidebar({ className, onNavigate, sessionName, sessionRole, logoutAction }: AdminSidebarProps) {
  ...
  const activeGroupTitle = useMemo(() => groups.find((group) => group.items.some((item) => pathname === item.href))?.title ?? "Geral", [pathname]);
  const [openGroup, setOpenGroup] = useState<string | null>(activeGroupTitle);

  const groupIsActive = (items: MenuItem[]) => items.some((item) => pathname === item.href);
